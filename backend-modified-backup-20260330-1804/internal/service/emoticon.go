...
type IEmoticonService interface {
	AddUserSysEmoticon(uid string, emoticonId int) error
	RemoveUserSysEmoticon(uid string, emoticonId int) error
	DeleteCollect(uid string, ids []int) error
}
...
func (s *EmoticonService) RemoveUserSysEmoticon(uid string, emoticonId int) error {
...
func (s *EmoticonService) AddUserSysEmoticon(uid string, emoticonId int) error {
	ids := s.EmoticonRepo.GetUserInstallIds(uid)
	if slices.Contains(ids, emoticonId) {
		return nil
	}
...
func (s *EmoticonService) DeleteCollect(uid string, ids []int) error {
