...
func (s *Service) toSplitForward(ctx context.Context, req ForwardMessageOpt) error {
	...
	if req.ToUserIdType == entity.ChatGroupMode {
		sequences := s.Sequence.BatchGet(ctx, repo.SequenceTypeGroup, req.ToUserId, len(messageItems))
		...
		sequence1 := s.Sequence.BatchGet(ctx, repo.SequenceTypeUser, req.ToUserId, len(messageItems))
		sequence2 := s.Sequence.BatchGet(ctx, repo.SequenceTypeUser, req.UserId, len(messageItems))
...
func (s *Service) toCombineForward(ctx context.Context, req ForwardMessageOpt) error {
	...
	case entity.ChatPrivateMode: // 好友发送消息
		...
		items = append(items, model.TalkUserMessage{
			MsgId:     strutil.NewMsgId(),
			OrgMsgId:  msgId,
			Sequence:  s.Sequence.Get(ctx, repo.SequenceTypeUser, req.ToUserId),
		...
		items = append(items, model.TalkUserMessage{
			MsgId:     strutil.NewMsgId(),
			OrgMsgId:  msgId,
			Sequence:  s.Sequence.Get(ctx, repo.SequenceTypeUser, req.UserId),
			MsgType:   entity.ChatMsgTypeForward,
			UserId:    req.UserId,
			ToFromId:  req.ToUserId,
			FromId:    req.UserId,
	...
	case entity.ChatGroupMode: // 向群发送消息
		record := model.TalkGroupMessage{
			MsgId:     strutil.NewMsgId(),
			Sequence:  s.Sequence.Get(ctx, repo.SequenceTypeGroup, req.ToUserId),
			MsgType:   entity.ChatMsgTypeForward,
			GroupId:   req.ToUserId,
			FromId:    req.UserId,
			IsRevoked: model.No,
...
func (s *Service) findUserNameList(ctx context.Context, uids []string) (map[string]string, error) {
