...
type IGroupService interface {
	Create(ctx context.Context, opt *GroupCreateOpt) (string, error)
	Update(ctx context.Context, opt *GroupUpdateOpt) error
	Dismiss(ctx context.Context, groupId string, uid string) error
	Secede(ctx context.Context, groupId string, uid string) error
	Invite(ctx context.Context, opt *GroupInviteOpt) error
	RemoveMember(ctx context.Context, opt *GroupRemoveMembersOpt) error
	List(userId string) ([]*model.GroupItem, error)
}
...
type GroupCreateOpt struct {
	...
	Avatar    string   // 群头像
	Profile   string   // 群简介
	MemberIds []string // 联系人ID
}

// Create 创建群聊
func (g *GroupService) Create(ctx context.Context, opt *GroupCreateOpt) (string, error) {
	var (
		err      error
	...
	err = g.Source.Db().Transaction(func(tx *gorm.DB) error {
		...
		record := &model.TalkGroupMessage{
			...
			Sequence:  g.Sequence.Get(ctx, repo.SequenceTypeGroup, group.Id),
...
// Dismiss 解散群组[群主权限]
func (g *GroupService) Dismiss(ctx context.Context, groupId string, uid string) error {
...
func (g *GroupService) Secede(ctx context.Context, groupId string, uid string) error {
	...
	record := &model.TalkGroupMessage{
		MsgId:     strutil.NewMsgId(),
		Sequence:  g.Sequence.Get(ctx, repo.SequenceTypeGroup, groupId),
...
func (g *GroupService) Invite(ctx context.Context, opt *GroupInviteOpt) error {
	...
	listHash := make(map[string]*model.TalkSession)
	...
	memberMaps := make(map[string]*model.Users)
	...
	record := &model.TalkGroupMessage{
		MsgId:     strutil.NewMsgId(),
		Sequence:  g.Sequence.Get(ctx, repo.SequenceTypeGroup, opt.GroupId),
...
func (g *GroupService) List(userId string) ([]*model.GroupItem, error) {
