...
type IGroupVoteService interface {
	Create(ctx context.Context, opt *GroupVoteCreateOpt) (string, error)
...
func (g *GroupVoteService) Create(ctx context.Context, opt *GroupVoteCreateOpt) (string, error) {
...
func (g *GroupVoteService) Submit(ctx context.Context, opt *GroupVoteSubmitOpt) error {
	...
	if count > 0 {
		return fmt.Errorf("重复投票[%s]", opt.VoteId)
	}
