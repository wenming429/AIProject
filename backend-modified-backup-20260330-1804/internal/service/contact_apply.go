...
type IContactApplyService interface {
	...
	List(ctx context.Context, uid string) ([]*model.ApplyItem, error)
	GetApplyUnreadNum(ctx context.Context, uid string) int
	ClearApplyUnreadNum(ctx context.Context, uid string)
}
...
func (s *ContactApplyService) Create(ctx context.Context, opt *ContactApplyCreateOpt) error {
	...
	s.Source.Redis().Incr(ctx, fmt.Sprintf("im:contact:apply:%s", opt.FriendId))
	return nil
}
...
func (s *ContactApplyService) Accept(ctx context.Context, opt *ContactApplyAcceptOpt) (*model.ContactApply, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		addFriendFunc := func(uid, fid string, remark string) error {
...
// List 联系人申请列表
func (s *ContactApplyService) List(ctx context.Context, uid string) ([]*model.ApplyItem, error) {
	fields := []string{
		"contact_apply.id",
...
func (s *ContactApplyService) GetApplyUnreadNum(ctx context.Context, uid string) int {

	num, err := s.Source.Redis().Get(ctx, fmt.Sprintf("im:contact:apply:%s", uid)).Int()
	if err != nil {
		return 0
	}
...
func (s *ContactApplyService) ClearApplyUnreadNum(ctx context.Context, uid string) {
	s.Source.Redis().Del(ctx, fmt.Sprintf("im:contact:apply:%s", uid))
}
