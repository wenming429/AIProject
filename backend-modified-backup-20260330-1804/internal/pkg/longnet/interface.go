...
type IAuthorize func(ctx context.Context, token string) (string, error)
...
type ISessionManager interface {
	...
	NewSession(uid string, conn IConn)          // 创建一个会话连接
	GetSession(connId int64) (ISession, error)  // 获取连接
	GetSessionNum() int32                       // 获取连接总数
	GetSessionUserNum() int32                   // 获取连接总数
	GetConnIds(uid string) []int64             // 获取用户在服务下的所有连接ID
	GetSessions(uid string) []ISession         // 获取用户在服务下的所有连接
...
type IHandler interface {
	...
	OnClose(connId int64, uid string)                          // 连接关闭事件
}
