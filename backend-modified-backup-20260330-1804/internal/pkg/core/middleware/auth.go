...
// IClaimsAuthId 定义了获取认证ID的方法
type IClaimsAuthId interface {
	GetAuthID() string
}
...
func NewJwtMiddleware[T IClaimsAuthId](
	secret []byte,
	storage IStorage,
...
func FormContext[T IClaimsAuthId](ctx context.Context) (T, error) {
...
func FormContextAuthId[T IClaimsAuthId](ctx context.Context) string {
	if ctx.Value(AuthClaimsKey{}) == nil {
		return ""
	}
