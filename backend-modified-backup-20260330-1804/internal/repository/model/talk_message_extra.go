...
type TalkRecordExtraText struct {
	...
	Mentions []string `json:"mentions,omitempty"` // @用户ID列表
}
...
type TalkRecordExtraForward struct {
	...
	ReceiverId string                         `json:"receiver_id"` // 接收者ID（群ID或者好友ID）
...
type TalkRecordExtraGroupCreate struct {
	OwnerId   string                       `json:"owner_id"`   // 操作人ID
...
type TalkRecordExtraTransferGroup struct {
	OldOwnerId   string `json:"old_owner_id"`   // 老群主ID
...
type TalkRecordExtraUserShare struct {
	UserId   string `json:"user_id"`  // 名片用户ID
