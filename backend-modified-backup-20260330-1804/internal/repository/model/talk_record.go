...
type TalkRecord struct {
	...
	UserId     string    `gorm:"column:user_id;" json:"user_id"`         // 发送者ID[0:系统用户;]
	ReceiverId string    `gorm:"column:receiver_id;" json:"receiver_id"` // 接收者ID（用户ID 或 群ID）
...
type TalkMessageRecord struct {
	...
	FromId    string    `json:"from_id"`    // 消息发送者
	ToFromId  string    `json:"to_from_id"` // 消息接受者
