...
func (c *Publish) Send(ctx *gin.Context) (any, error) {
	...
	if err := c.AuthService.IsAuth(ctx.Request.Context(), &service.AuthOption{
		...
		UserId:            uid,
		ToFromId:          strconv.Itoa(in.ToFromId),
...
func (c *Publish) onSendText(ctx *gin.Context) error {
	...
	// 将 []int 转换为 []string
	mentions := make([]string, len(in.Body.Mentions))
	for i, v := range in.Body.Mentions {
		mentions[i] = strconv.Itoa(v)
	}
...
func (c *Publish) onSendForward(ctx *gin.Context) error {
	...
	uid := middleware.FormContextAuthId[entity.WebClaims](ctx.Request.Context())
	go func() {
		// 转换 GroupIds 为 []string
		gids := make([]string, len(in.Body.GroupIds))
		for i, v := range in.Body.GroupIds {
			gids[i] = strconv.Itoa(v)
		}
		// 转换 UserIds 为 []string
		uids := make([]string, len(in.Body.UserIds))
		for i, v := range in.Body.UserIds {
			uids[i] = strconv.Itoa(v)
		}
...
func (c *Publish) onSendCard(ctx *gin.Context) error {
	...
	err := c.MessageService.CreateBusinessCardMessage(ctx.Request.Context(), message.CreateBusinessCardMessage{
		...
		FromId:   uid,
		ToFromId: strconv.Itoa(in.ToFromId),
		UserId:   strconv.Itoa(in.Body.UserId),
	})
	if err != nil {
		return ctx.Error(err)
	}
