...
func (a Apply) Create(ctx context.Context, in *web.GroupApplyCreateRequest) (*web.GroupApplyCreateResponse, error) {
	...
	// Convert int32 GroupId to string for database
	groupId := fmt.Sprintf("%d", in.GroupId)

	apply, err := a.GroupApplyRepo.FindByWhere(ctx, "group_id = ? and user_id = ? and status = ?", groupId, uid, model.GroupApplyStatusWait)
	...
	find, err := a.GroupMemberRepo.FindByWhere(ctx, "group_id = ? and leader = ?", groupId, model.GroupMemberLeaderOwner)
	if err == nil && find != nil {
...
func (a Apply) Agree(ctx context.Context, in *web.GroupApplyAgreeRequest) (*web.GroupApplyAgreeResponse, error) {
	...
	apply, err := a.GroupApplyRepo.FindById(ctx, fmt.Sprintf("%d", in.ApplyId))
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, err
	}
...
func (a Apply) List(ctx context.Context, in *web.GroupApplyListRequest) (*web.GroupApplyListResponse, error) {
	...
	if !a.GroupMemberRepo.IsLeader(ctx, groupId, uid) {
		return nil, entity.ErrPermissionDenied
	}

	list, err := a.GroupApplyRepo.List(ctx, []string{groupId})
	if err != nil {
		return nil, err
	}
	...
	for _, item := range list {
		items = append(items, &web.GroupApplyListResponse_Item{
			Id:        int32(item.Id),
			UserId:    in.GroupId, // 使用请求中的 int32 值
			GroupId:   in.GroupId,
			Remark:    item.Remark,
			Avatar:    item.Avatar,
...
func (a Apply) All(ctx context.Context, req *web.GroupApplyAllRequest) (*web.GroupApplyAllResponse, error) {
	...
	groupMap := sliceutil.ToMap(groups, func(t *model.Group) string {
		return t.Id
	})
	...
	for _, item := range list {
		resp.Items = append(resp.Items, &web.GroupApplyAllResponse_Item{
			Id:        int32(item.Id),
			UserId:    func() int32 { id, _ := strconv.Atoi(item.UserId); return int32(id) }(),
			GroupName: groupMap[item.GroupId].Name,
			GroupId:   func() int32 { id, _ := strconv.Atoi(item.GroupId); return int32(id) }(),
			Remark:    item.Remark,
			Avatar:    item.Avatar,
			Nickname:  item.Nickname,
			CreatedAt: timeutil.FormatDatetime(item.CreatedAt),
