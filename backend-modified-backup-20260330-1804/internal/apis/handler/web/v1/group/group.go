...
func (g Group) List(ctx context.Context, in *web.GroupListRequest) (*web.GroupListResponse, error) {
	...
	for _, item := range items {
		...
		creatorId, _ := strconv.Atoi(item.CreatorId)
...
func (g Group) Create(ctx context.Context, in *web.GroupCreateRequest) (*web.GroupCreateResponse, error) {
	...
	for _, id := range sliceutil.Unique(in.UserIds) {
		memberIds = append(memberIds, strconv.Itoa(int(id)))
	}
	...
	if len(memberIds)+1 > model.GroupMemberMaxNum {
	...
	return &web.GroupCreateResponse{GroupId: int32(id)}, nil
}

func (g Group) Detail(ctx context.Context, in *web.GroupDetailRequest) (*web.GroupDetailResponse, error) {
	uid := middleware.FormContextAuthId[entity.WebClaims](ctx)

	groupInfo, err := g.GroupRepo.FindById(ctx, strconv.Itoa(int(in.GroupId)))
	if err != nil {
		return nil, err
	}

	if groupInfo.Id == "" || groupInfo.Id == "0" {
		return nil, entity.ErrGroupNotExist
	}

	groupIdInt, _ := strconv.Atoi(groupInfo.Id)
	resp := &web.GroupDetailResponse{
		...
		VisitCard: g.GroupMemberRepo.GetMemberRemark(ctx, strconv.Itoa(int(in.GroupId)), uid),
	...
	if g.TalkSessionRepo.IsDisturb(uid, int(in.GroupId), 2) {
		resp.IsDisturb = 1
	}
...
func (g Group) MemberList(ctx context.Context, in *web.GroupMemberListRequest) (*web.GroupMemberListResponse, error) {
	uid := middleware.FormContextAuthId[entity.WebClaims](ctx)
	group, err := g.GroupRepo.FindById(ctx, strconv.Itoa(int(in.GroupId)))
	if err != nil {
		return nil, err
	}
	...
	if !g.GroupMemberRepo.IsMember(ctx, strconv.Itoa(int(in.GroupId)), uid, false) {
		return nil, entity.ErrPermissionDenied
	}

	list := g.GroupMemberRepo.GetMembers(ctx, strconv.Itoa(int(in.GroupId)))
...
func (g Group) Dismiss(ctx context.Context, in *web.GroupDismissRequest) (*web.GroupDismissResponse, error) {
	uid := middleware.FormContextAuthId[entity.WebClaims](ctx)
	groupIdStr := strconv.Itoa(int(in.GroupId))
	if !g.GroupMemberRepo.IsMaster(ctx, groupIdStr, uid) {
		return nil, entity.ErrPermissionDenied
	}

	if err := g.GroupService.Dismiss(ctx, groupIdStr, uid); err != nil {
		return nil, err
	}
...
func (g Group) GetInviteFriends(ctx context.Context, in *web.GetInviteFriendsRequest) (*web.GetInviteFriendsResponse, error) {
	...
	mids := g.GroupMemberRepo.GetMemberIds(ctx, strconv.Itoa(int(in.GroupId)))
	if len(mids) == 0 {
		return &web.GetInviteFriendsResponse{
			Items: data,
		}, nil
...
func (g Group) RemoveMember(ctx context.Context, in *web.GroupRemoveMemberRequest) (*web.GroupRemoveMemberResponse, error) {
	uids := make([]string, 0)
	for _, id := range sliceutil.Unique(in.UserIds) {
		uids = append(uids, strconv.Itoa(int(id)))
	}
	...
	uid := middleware.FormContextAuthId[entity.WebClaims](ctx)
	groupIdStr := strconv.Itoa(int(in.GroupId))
	if !g.GroupMemberRepo.IsLeader(ctx, groupIdStr, uid) {
		return nil, entity.ErrPermissionDenied
	}
