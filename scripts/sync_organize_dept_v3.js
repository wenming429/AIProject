/**
 * UDMOrganization → Organize_Dept 同步脚本 (逗号分隔版)
 * 
 * ancestors 格式：
 *   - 根节点: "0"
 *   - 子节点: "0,1" (根节点ID, 父节点ID)
  ...
  try {
    ...
    for (const root of rootNodes) {
      ...
      for (const node of descendants) {
        allNodes.set(node.ID, { ...node, sortOrder: node.INNERORDER || 0 });
        totalCount++;
      }
    }
...
function buildTreeAndAssignIds(roots, allNodes) {
  logger.info('步骤4: 构建树结构（ancestors 使用逗号分隔）...');
  ...
  // 递归分配 ID
  function assignIds(parentId, parentAncestorsStr) {
    ...
    for (const childId of childIds) {
      ...
      // 构建 ancestors：
      // 根节点的 ancestors = "0"
      // 子节点的 ancestors = "0,1" (父节点的ancestors + 父节点的targetId)
      let ancestors;
      if (parentId === 'ROOT') {
        ancestors = '0';
      } else {
      ...
      const level = ancestors.split(',').length - 1;
...
async function batchInsertData(nodeMap, isDryRun) {
  ...
  try {
    ...
    for (const node of sortedNodes) {
      ...
      // 状态：EFFECTIVESTATUS='A' -> status=1
      const status = 1;
      
      // order_num 使用 INNERORDER（整数）
      let orderNum = parseInt(node.INNERORDER) || 0;
      if (orderNum === 0 || orderNum > 10000000) {
        orderNum = node.targetId;  // 使用 dept_id 作为序号
      }
      
      batchData.push([
        parentTargetId,
        node.ancestors,                    // ancestors: "0" 或 "0,1" 或 "0,1,2"
        node.FULLNAME || '',
        orderNum,
        '',
        '',
        ...
        node.ID,
        node.PARENTID || null,
        node.EFFECTIVEDATE || new Date(),
        node.MASTERDATA_BATCHTIME || new Date()
      ]);
...
async function validateResult() {
  ...
  try {
    ...
    const [samples] = await mysqlPool.query(`
      ...
      FROM organize_dept
      WHERE parent_id = 0 OR ancestors = '0' OR ancestors LIKE '0,%'
    ...
    const [sortCheck] = await mysqlPool.query(`
      SELECT d1.dept_name as parent_name, d2.dept_id, d2.dept_name, d2.order_num
...
async function main() {
  ...
  console.log('排序: 按 INNERORDER 升序');
  console.log('='.repeat(60) + '\n');
